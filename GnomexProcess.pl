...
tools::mongo::addPipelineSamples( sh => $pl->{samples} );
$pl->multiprocessQ(sort keys %{$pl->{fastqc}->{commands}});
$pl->multiprocessQ(sort keys %{$pl->{align}->{commands}});
...
## this adds FS locations to each file filed
sub findFileLocations {
	...
	for my $samp ( sort keys %{$self->{samples}} ) {
		...
		for my $read ( sort keys %{$self->{samples}->{$samp}->{files}} ){
			...
			my $file = tools::mongo::checkSampFileDB(
					samp => $samp,
					read => $read
				);
			...
			if (defined($file) && $file ne "0"){
				...
				if (my $path = findFastQPathSample(file => $fname, exp => $exp) ){
					...
					if ($self->isFastQ($path) ){
						...
						$msg = "$path is not a fastq file";
						print "$msg\n";
						# clear out read name information
						$self->{samples}->{$samp}->{files}->{$read}->{name} = 0;
						$self->{samples}->{$samp}->{files}->{$read}->{error} = "$path is not a fastq file";
						$self->errLog ($msg);
						$errMsg .= "$msg; ";
					}
				} else {
					## could not be found on fs
					my $msg = "$fname could not be found on the filesystem\n";
...
sub buildFastQCCommandHash{
	...
	for my $samp ( sort keys %{ $self->{samples} } ){
		...
		if (-e $outdir."/".$read1base."_fastqc.zip"){
			...
			$self->{fastqc}->{commands}->{$command} = 1;
			$succMsg = $command;
		}
		if ($self->isPairedEnd){
...
sub buildAlignCommandHash{
	...
	for my $samp ( sort keys %{ $self->{samples} }){
		...
		unless ( exists ($self->{align}->{force}) && $self->{align}->{force} ) {
			...
			if (glob("$analysisDir/$exp/$pattern1*--$samp/$pattern2")){
